...
> At a large enterprise I'm familiar with, the network team had a single 4,000-line playbook that had both inventory information and task logic mixed together — device IPs hardcoded in the playbook itself, credentials in variables inside the play, task logic referencing specific hostnames directly. When they needed to run the same automation against a second data center, they had to copy the entire playbook and manually edit every IP address and hostname. A proper inventory separation would have meant just adding a second inventory file for DC2 and running the same playbook with `-i inventory/dc2/hosts.yml`. The playbook itself wouldn't change at all.

---

## 7.2 — INI vs YAML Inventory Formats
...
| Feature | INI | YAML |
|---|---|---|
| Readability for small inventories | ✅ Simpler, less indentation | ⚠️ More verbose |
...
The rest of this guide uses YAML inventory.
...
> ```
> The `-y` flag outputs in YAML format. I always review the output — automatic conversion sometimes flattens nested group structures that need to be manually reorganized.

---

## 7.3 — Static Inventory: Building the Full Enterprise Lab Inventory

Part 6's inventory was a starting point. Now I build it out properly — full group hierarchy, logical groupings, and a structure that mirrors how I'll actually target devices in playbooks.
...
This matters for variable inheritance (covered in 7.5) and for playbook targeting. If I write `hosts: cisco_nxos`, my playbook runs against `spine-01`, `spine-02`, `leaf-01`, and `leaf-02` — all devices in the hierarchy beneath `cisco_nxos`.

### ### ℹ️ Info

> The `all` group always exists in Ansible, even if I don't define it explicitly. Every host in any inventory is automatically a member of `all`. This is why `group_vars/all.yml` is the place for truly global variables — they apply to every single device regardless of what other groups they belong to. I use `all` for things like NTP servers, DNS servers, and global credentials that are the same across every platform.
...
> The `!` operator excludes a group. `hosts: all,!linux_hosts` means every device in `all` except those in `linux_hosts`. I use this when I want to run a playbook against the entire network except a specific subset — for example, running a backup playbook against all network devices but not the Linux hosts which use a different backup mechanism.

---

## 7.4 — Host Variables (`ansible_host`, `ansible_user`, `ansible_network_os`)
...
### Where to Set Variables — The Decision
...
> Putting passwords and credentials directly inside `hosts.yml` is a common beginner pattern that creates a security problem: the inventory file gets committed to Git with credentials in plain text. The correct pattern from the start is to keep credentials in `group_vars/` files where Ansible Vault (Part 12) can encrypt just the sensitive values. I'm using plain text in this lab for simplicity, but Part 12 will vault all of these.

---

## 7.5 — Group Variables and the `group_vars/` Directory
...
---
# =============================================================
# Global variables — applied to EVERY device in the inventory
# =============================================================

# --- Credentials (will be vaulted in Part 12) ---
ansible_user: ansible
ansible_password: "ansible123"
...
# --- SNMP Community (will be vaulted in Part 12) ---
...
### `group_vars/cisco_ios.yml` — IOS-XE Connection Settings
...
---
# =============================================================
# Cisco IOS / IOS-XE — Connection and platform settings
# =============================================================

ansible_network_os: cisco.ios.ios
ansible_connection: network_cli

# Privilege escalation — enter enable mode
ansible_become: true
ansible_become_method: enable
ansible_become_password: "ansible123"    # Will be vaulted in Part 12
...
### `group_vars/cisco_nxos.yml` — NX-OS Connection Settings
...
---
# =============================================================
# Cisco NX-OS — Connection and platform settings
# =============================================================

ansible_network_os: cisco.nxos.nxos
ansible_connection: network_cli

# NX-OS does not use enable mode — already at privilege level
...
# These will be referenced in the NX-OS base config role (Part 13)
...
### `group_vars/spine_switches.yml` — Spine-Specific Variables
...
### `group_vars/leaf_switches.yml` — Leaf-Specific Variables
...
### `group_vars/paloalto.yml` — PAN-OS Connection Settings
...
### `group_vars/wan.yml` — WAN Group Variables
...
### `group_vars/linux_hosts.yml` — Linux Host Connection Settings
...
> When a host belongs to multiple groups (e.g., `spine-01` belongs to both `spine_switches` and `cisco_nxos`), it inherits variables from all of them. If the same variable is defined in multiple group files, Ansible resolves the conflict using **variable precedence** — more specific groups win over less specific ones. `host_vars/` always wins over `group_vars/`. The full 18-level variable precedence order is covered in Part 11.

---

## 7.6 — Host Variables and the `host_vars/` Directory

While `group_vars/` handles settings shared across a group, `host_vars/` is for variables that are unique to a specific device — its loopback IP, its BGP AS number, its specific interface assignments. I build realistic `host_vars` files for one device from each platform.
...
### `host_vars/wan-r1.yml` — IOS-XE WAN Router
...
### `host_vars/wan-r2.yml` — IOS-XE WAN Router 2
...
### `host_vars/fw-01.yml` — Palo Alto PAN-OS Firewall
...
### `host_vars/spine-01.yml` — NX-OS Spine Switch 1
...
### `host_vars/leaf-01.yml` — NX-OS Leaf Switch 1
...
## 7.8 — Dynamic Inventory — Concepts and Preview
...
Netbox is the industry-standard network source of truth (covered fully in Part 26). When Netbox is running, I replace (or supplement) my static `hosts.yml` with a Netbox inventory plugin config.

Here's what that config file looks like — I'm creating it now even though Netbox isn't running yet, so the structure is familiar when I get to Part 26:
...
For this guide, I use static inventory through Part 25. Part 26 migrates to Netbox dynamic inventory.

### ### 🏢 Real-World Scenario

> I've worked with teams at two different enterprises that had the same problem: their static Ansible inventory was months out of date. Devices had been decommissioned but were still in the inventory (causing failed connection attempts on every playbook run). New devices had been added but not yet in the inventory (so they were never automated). The inventory was a source of constant friction. The fix in both cases was Netbox as the source of truth, with the dynamic inventory plugin. The moment a device was added to Netbox with the right platform and site tags, it was automatically available in Ansible. The moment it was decommissioned in Netbox, it disappeared from the inventory. Zero manual inventory maintenance.

---

## 7.9 — Testing the Inventory
...
## 7.10 — Common Gotchas in This Section

### ### 🪲 Gotcha — Host defined in multiple groups causes duplicate task runs

If I define `wan-r1` as a host in both `cisco_ios` and `wan_edge` groups directly (not through group nesting), Ansible deduplicates automatically — the task runs once per host, not once per group membership. But if I use `add_host` in a playbook or have the same host in two completely separate inventory files, I can end up with truly duplicate entries. I verify with:

```bash
ansible-inventory --list | grep -c "wan-r1"
```

If this returns more than one, I have a duplication problem to resolve.

### ### 🪲 Gotcha — `group_vars` files not being read

`group_vars/` must be in the same directory as the inventory file, OR in the playbook's directory. If I put `hosts.yml` in `inventory/` but `group_vars/` is in the project root, Ansible won't find the group vars.

```
# ✅ Correct structure — group_vars alongside the inventory
inventory/
├── hosts.yml
├── group_vars/
│   ├── all.yml
...
# ❌ Wrong — group_vars in wrong location
inventory/hosts.yml
group_vars/all.yml     ← Not alongside hosts.yml, won't be read
```

Alternatively, `group_vars/` in the **playbook directory** is also read. Both locations work — I pick one and stay consistent.

### ### 🪲 Gotcha — YAML boolean and integer parsing in inventory

YAML automatically converts some values to non-string types:

```yaml
# These are NOT strings — YAML parses them as booleans and integers
ansible_become: yes        # Boolean True
ansible_port: 22           # Integer 22
ansible_httpapi_use_ssl: true   # Boolean True

# This is a string
ansible_host: "172.16.0.11"    # Quoted = string (safe either way for IPs)
```

For most Ansible variables this doesn't matter — Ansible handles type coercion. But when using variables in Jinja2 string concatenation, unexpected types cause errors. When in doubt, quote string values explicitly.

### ### 🪲 Gotcha — `ansible_network_os` wrong format causes "unable to determine network OS"

The `ansible_network_os` value must match the collection's full namespace format:

```yaml
# ❌ Wrong — old format, will fail with newer collections
ansible_network_os: ios
ansible_network_os: nxos

# ✅ Correct — full collection namespace format
ansible_network_os: cisco.ios.ios
ansible_network_os: cisco.nxos.nxos
...
Older documentation and blog posts often use the short form — it worked in older Ansible versions but fails with current collection-based installations.

---

## 7.11 — Committing the Expanded Inventory to Git

```bash
cd ~/projects/ansible-network

git add inventory/
git status    # Review all the new and modified files
...
The inventory is now a proper data source — not just a list of IPs but a structured representation of the entire network with per-device variables, platform groups, logical groups, and the foundation for future dynamic inventory. Part 8 takes all of this and organizes the entire project directory the right way.
