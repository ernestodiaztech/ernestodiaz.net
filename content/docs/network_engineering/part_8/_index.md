...
By the end of Part 7, the project directory has grown organically as files were added as each part needed them. That's fine for learning, but now I step back and look at the whole structure deliberately. This part locks in the definitive project layout, expands `ansible.cfg` with every setting explained, establishes naming conventions I'll follow for the rest of the lab, and shows exactly how to separate playbooks by function so the project stays navigable as it grows.

---

## The Definitive Project Structure
...
**`roles/`** - Reusable, self-contained units of automation. A role for IOS base config can be called from any playbook.
...
**`templates/`** - Jinja2 (`.j2`) template files. Organized by platform. Templates reference variables from inventory to generate device-specific configurations.
...
Part 5 created a working `ansible.cfg`. Now I expand it into the definitive version for this lab with every setting explained, lab vs. production differences called out explicitly.
